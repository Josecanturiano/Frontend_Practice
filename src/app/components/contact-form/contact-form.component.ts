...
export class ContactFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.contactId = Number(this.activatedRouter.snapshot.paramMap.get("id"));
    ...
    if(this.contactId){
      ...
      this.contact = this.contactService.findContactById(this.contactId);

      this.contactForm.patchValue(this.contact)
      this.phoneNumbers = this.contact.phoneNumbers;
      this.img = this.contact.photo;
    }else{
  ...
  public save() : void {    
    ...
    if ((this.contactForm.valid && this.phoneNumbers.length > 0)){
      ...
      if(this.isNewContact){
        ...
        this.contact.photo = this.contactForm.value['photo'];
